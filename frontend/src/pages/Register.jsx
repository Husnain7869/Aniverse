...
export default function Register() {
  ...
  const { register } = useAuthStore(); const navigate = useNavigate();
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#faf5ff 0%,#f3e8ff 50%,#ede9fe 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ width:"100%", maxWidth:420 }}>
        <div style={{ textAlign:"center", marginBottom:36 }}>
          ...
          <div style={{ fontSize:13, color:"var(--text-muted)", marginTop:4 }}>Begin your anime journey</div>
        </div>
        <div style={{ background:"var(--surface)", borderRadius:24, padding:36, boxShadow:"0 8px 40px rgba(124,58,237,0.12)", border:"1px solid var(--border)" }}>
          <h2 style={{ fontSize:20, fontWeight:800, color:"var(--text-primary)", marginBottom:24 }}>Create your account</h2>
          ...
          <form onSubmit={submit} style={{ display:"flex", flexDirection:"column", gap:16 }}>
            {[["Username","text","username"],["Email","email","email"],["Password","password","password"]].map(([label,type,key])=>(
              <div key={key}>
          ...
          <p style={{ textAlign:"center", marginTop:22, fontSize:13, color:"var(--text-muted)" }}>
            Have an account?{" "}<Link to="/login" style={{ color:"var(--purple-600)", fontWeight:700 }}>Sign in →</Link>
          </p>
        </div>
      </div>
    </div>
