...
function FloatingCard({ cover, title, style }) {
  return (
    <div style={{
      position: "absolute", width: 130, borderRadius: 16, overflow: "hidden",
      boxShadow: "0 20px 50px rgba(109,40,217,0.25), 0 4px 16px rgba(0,0,0,0.15)",
      ...style,
    }}>
      {cover
...
export default function Register() {
  ...
  const submit = async (e) => {
    ...
    catch (e) { setErr(e.response?.data?.detail || "Registration failed. Please try again."); }
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "80px 20px 60px", position: "relative", zIndex: 5 }}>
        <div style={{ width: "100%", maxWidth: 420, background: "#fff", borderRadius: 28, padding: "40px 40px 36px", boxShadow: "0 24px 80px rgba(109,40,217,0.14), 0 4px 24px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13.5, color: "#6b7280", fontWeight: 500 }}>Join AniVerse and elevate your anime experience.</p>
          </div>
          ...
          <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            {/* Username */}
            <div>
              <label style={{ fontSize: 11, fontWeight: 800, color: "#374151", display: "block", marginBottom: 7, letterSpacing: "0.8px", textTransform: "uppercase" }}>Username</label>
              <div style={{ position: "relative" }}>
                <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", display: "flex" }}><IcUser /></span>
                <input className="auth-input" type="text" placeholder="Choose a username" value={form.username}
                  onChange={e => setForm({ ...form, username: e.target.value })} required style={inputStyle} />
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input className="auth-input" type="email" placeholder="Enter your email" value={form.email}
            ...
            {/* Password */}
            <div>
              <label style={{ fontSize: 11, fontWeight: 800, color: "#374151", display: "block", marginBottom: 7, letterSpacing: "0.8px", textTransform: "uppercase" }}>Password</label>
              <div style={{ position: "relative" }}>
                <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", display: "flex" }}><IcLock /></span>
                <input className="auth-input" type={showPass ? "text" : "password"} placeholder="Create a password" value={form.password}
                  onChange={e => setForm({ ...form, password: e.target.value })} required minLength={8} style={{ ...inputStyle, paddingRight: 44 }} />
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 7 }}>
                <IcShield />
                <span style={{ fontSize: 12, color: "#6b7280", fontWeight: 600 }}>At least 8 characters</span>
              </div>
            </div>
            ...
            <button type="submit" className="auth-btn-primary" disabled={loading} style={{
              ...
              {loading ? "Creating account…" : <><span>Create Account</span><IcArrow /></>}
            </button>
          </form>
          ...
          <p style={{ textAlign: "center", marginTop: 24, fontSize: 13.5, color: "#6b7280", fontWeight: 500 }}>
            ...
            <span onClick={() => navigate("/login")} style={{ color: "#7c3aed", fontWeight: 800, cursor: "pointer" }}>
              Sign in →
            </span>
          </p>
        </div>
